...
def val_step(model, x, y, batch_size):
    ...
    with torch.no_grad():
      for i, (batch_idx, batch_x, batch_y) in enumerate(batchify(x, y, batch_size)):
          pred, attention_per_head = model(batch_x, batch_y)
          loss = model._loss(batch_y)
          total_loss += loss.item()
    ...
    return total_loss / (i + 1), idx, y_gold, y_pred, attention_weights
...
def learn(model, x, y, tag2idx, val_fold, args, idx_order = [], original_docs = {}):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, val_idx, val_gold, val_pred, _ = val_step(model, val_x, val_y, args.batch_size)
